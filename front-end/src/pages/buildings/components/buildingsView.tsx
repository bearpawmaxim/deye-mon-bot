...
export const BuildingsView: FC<BuildingsViewProps> = ({ loading, isAuthenticated, buildings }) => {
  if (loading) {
    return (
  ...
  return <SimpleGrid cols={{ base: 1, sm: 2, lg: 3 }} spacing="xl">
    {buildings.map((building, idx) => isAuthenticated
      ? <EditableBuildingCard key={`b_${idx}`} building={building as BuildingListItem} />
      : <BuildingCard key={`b_${idx}`} building={building as BuildingListItem} />)}
